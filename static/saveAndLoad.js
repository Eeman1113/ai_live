            ...
            request.onsuccess = async () => {
                if (request.result) {
                    ...
                    if (loadUI(UI_UI_CURWORLD < 0)) {
                        ...
                        await p;
                        // Force ai_live defaults on existing saves
                        saveGD(UI_VIEWMODE_SELECT, UI_VIEWMODE_NORMAL);
                        saveGD(UI_NAME, "ai_live");
                    ...
                    await promise;
                    // Hide toolbars for clean vertical-slice view
                    saveGD(UI_TOPBAR_MAINMENU, false);
                    saveGD(UI_TOPBAR_BLOCK, false);
                    saveGD(UI_TOPBAR_LIGHTING, false);
                    saveGD(UI_TOPBAR_VIEWMODE, false);
...
export async function createNewWorld() {
    ...
    // Auto-generate terrain with grass
    generateVerticalSliceTerrain();

    saveGD(UI_CANVAS_VIEWPORT_CENTER_X, 100);
    saveGD(UI_CANVAS_VIEWPORT_CENTER_Y, 80);
    saveCurGame();
}

// Generate a natural terrain cross-section with soil, rock, and grass
function generateVerticalSliceTerrain() {
    const width = 200;
    const baseDepth = 30;
    const surfaceY = 95;

    // Rolling heightmap from layered sine waves
    const heights = [];
    for (let x = 0; x < width; x++) {
        let h = Math.sin(x * 0.02) * 8
              + Math.sin(x * 0.05 + 1.3) * 4
              + Math.sin(x * 0.11 + 2.7) * 2
              + Math.sin(x * 0.23 + 0.5) * 1;
        heights.push(Math.round(surfaceY + h));
    }
    ...
    for (let x = 0; x < width; x++) {
        ...
        for (let depth = 0; depth < baseDepth; depth++) {
            ...
            if (depth < 12) {
                ...
                addSquare(new SoilSquare(x, y));
            } else if (depth < 22) {
                // Transition: mix of soil and rock
                if (Math.random() < 0.25 + depth * 0.025) {
                    addSquare(new RockSquare(x, y));
                } else {
                    addSquare(new SoilSquare(x, y));
                }
            } else {
        ...
        // Scatter grass seeds on surface
        if (Math.random() < 0.15) {
            const seedSq = addSquare(new SeedSquare(x, surface - 1));
            if (seedSq) {
                new OriginGrassSeedOrganism(seedSq, [Math.random()]);
            }
        }
    }

    // Add water in low spots
    for (let x = 1; x < width - 1; x++) {
        if (heights[x] >= heights[x - 1] && heights[x] >= heights[x + 1] && Math.random() < 0.04) {
            addSquare(new WaterSquare(x, heights[x] - 1));
        }
    }
}
